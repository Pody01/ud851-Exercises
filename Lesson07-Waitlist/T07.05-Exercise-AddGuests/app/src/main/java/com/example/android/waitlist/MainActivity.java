...
//        TestUtil.insertFakeData(mDb);
    ...
    public void addToWaitlist(View view) {
        ...
        // TODO (12) Make sure you surround the Integer.parseInt with a try catch and log any exceptio
        try {
            count = Integer.parseInt(mNewPartySizeEditText.getText().toString());
        } catch (Exception ex) {
            // noting
        }
        // TODO (14) call addNewGuest with the guest name and party size
        addGuest(mNewGuestNameEditText.getText().toString(), count);
    ...
    private void addGuest(String name, int count) {
        ...
        cv.put(WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE, count);
        // TODO (8) call insert to run an insert query on TABLE_NAME with the ContentValues created
        mDb.insert(WaitlistContract.WaitlistEntry.TABLE_NAME,
                null, cv);
    }
